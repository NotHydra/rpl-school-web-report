...
class Excel:
    ...
    def change_sheet(self, active_sheet: int or str):
        if type(active_sheet) == int:
            self.active_sheet = self.workbook[
                self.workbook.sheetnames[active_sheet - 1]
            ]
    ...
    def adjust_width(
        ...
        merged_cell_range = [
            str(merged_cell).split(":")
            for merged_cell in self.active_sheet.merged_cells
        ]
            ...
            if width_limit > 0:
                if max_width_value > width_limit:
                    ...
                    self.alignment_multiple(
                        [column_number, 1], [column_number, len(column)], wrap=True
                    )
    ...
    def change_cell_number_format_multiple(
        self, start_range: any, end_range: any, number_format: str
    ):
        start_column, start_row = Excel.convert_range(start_range)
        end_column, end_row = Excel.convert_range(end_range)
    ...
    def write_value_singular(self, range: any, value: any):
        if type(value) == list:
            raise TypeError(
                "Use write_value_multiple function if the value type is a list"
            )
        ...
        if type(value) == list:
            for check_value in value:
                if type(check_value) == list:
                    raise TypeError(
                        "Use write_value_multiple_2d function if the value is a 2D list"
                    )
            ...
            for row in range(start_row, end_row + 1):
                for column in range(start_column, end_column + 1):
                    self.active_sheet.cell(
                        row=row, column=column, value=value[value_counter]
                    )
                    value_counter += 1
    ...
    def write_value_multiple_2d(self, start_range: any, value: any):
        if type(value) == list:
            ...
            if value_is_valid:
                start_column, start_row = Excel.convert_range(start_range)
                end_column, end_row = start_column + len(value[0]), start_row + len(
                    value
                )
                ...
                for x, row in enumerate(range(start_row, end_row)):
                    for y, column in enumerate(range(start_column, end_column)):
                        self.active_sheet.cell(
                            row=row, column=column, value=value[x][y]
                        )
    ...
    def unmerge(self, start_range: any, end_range: any):
        ...
        self.active_sheet.unmerge_cells(
            start_row=start_row,
            start_column=start_column,
            end_row=end_row,
            end_column=end_column,
    ...
    def font_attributes(**attributes: any):
        ...
        if "underline" in attributes:
            if type(attributes.get("underline")) == str:
                ...
                underline_name = (
                    underline_name[0].lower() + underline_name[1:]
                ).replace(" ", "")
    ...
    def fill_singular(self, cell_range: any, **attributes: any):
        ...
        self.active_sheet.cell(row=row, column=column).fill = eval(
            f"PatternFill({attributes_string})"
        )
            ...
            for row in range(start_row, end_row + 1):
                for column in range(start_column, end_column + 1):
                    if column % 2 != 0:
                        self.active_sheet.cell(
                            row=row, column=column
                        ).fill = main_pattern_fill

                    elif column % 2 == 0:
                        self.active_sheet.cell(
                            row=row, column=column
                        ).fill = second_pattern_fill
    ...
    def border_set(side, attribute):
        ...
        if side == "all":
            border = Border(
                top=attribute, left=attribute, right=attribute, bottom=attribute
            )
    ...
    def border_multiple(
        self, start_range: any, end_range: any, side: str, **attributes: any
    ):
        start_column, start_row = Excel.convert_range(start_range)
        end_column, end_row = Excel.convert_range(end_range)
    ...
    def alignment_attributes(**attributes: any):
        ...
        if "horizontal" in attributes:
            if type(attributes.get("horizontal")) == str:
                ...
                horizontal_type = (
                    horizontal_type[0].lower() + horizontal_type[1:]
                ).replace(" ", "")
    ...
    def excel_if(
        self, range1: any, logic: str, range2: any, return1: any, return2: any
    ):
    ...
    def summary_if(
        self,
        start_criteria_range: any,
        end_criteria_range: any,
        criteria: str,
        sum_start_range: any,
        sum_end_range: any,
    ):
        criteria_array = self.get_value_multiple(
            start_criteria_range, end_criteria_range
        )
    ...
    def average_if(
        ...
        total_value = self.summary_if(
            start_criteria_range,
            end_criteria_range,
            criteria,
            sum_start_range,
            sum_end_range,
